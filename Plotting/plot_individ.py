...
levels_num = [1e-7,1e-5,1e-3,1e-1,1e1,1e3]
...
im3 = ax.contourf(X3,Y3,p_fe, norm=LogNorm(),levels=levels_num, cmap='Reds', alpha=0.5)
im4 = ax.contourf(X4,Y4,p_mg, norm=LogNorm(),levels=levels_num, cmap='Purples', alpha=0.5)
im5 = ax.contourf(X5,Y5,p_cr+p_pure_cr, norm=LogNorm(),levels=levels_num, cmap='Oranges', alpha=0.5)
...
plt.savefig(path+'/layers_num_'+str(name)+'.pdf', format = 'pdf', bbox_inches='tight')
plt.close()

levels_mass = [1e-13,1e-11,1e-9,1e-7,1e-5]
...
ax.set_ylabel(r'Pressure [bar]')
ax.set_xlabel(r'Particle Radius [$\mu$m]')
plt.savefig(path+'/layers_mass_'+str(name)+'.pdf', format = 'pdf', bbox_inches='tight')
plt.close()
